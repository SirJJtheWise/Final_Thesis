...
def main():
    ...
    #Todo here we will pass the anchors to the Merged_Model which will create its own costom

    if args.arch == 'Custom':
        #we inserted out modelö here
        from merged_models import MergedModel
        model = MergedModel()
    else:
...
def train(train_loader, model, optimizer, lr_scheduler, epoch, cfg):
    ...
    avg = AverageMeter()
    model.train()
    model.module.dimppart.eval()
    model.module.dimppart.apply(BNtoFixed)

    model.module.rest.train()
    model = model.cuda()
    end = time.time()
