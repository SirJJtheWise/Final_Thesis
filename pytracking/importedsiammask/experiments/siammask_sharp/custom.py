from importedsiammask.models.siammask_sharp import SiamMask
from importedsiammask.models.features import MultiStageFeature
from importedsiammask.models.rpn import RPN, DepthCorr
from importedsiammask.models.mask import Mask
import torch
import torch.nn as nn
import torch.nn.functional as F
from importedsiammask.utils.load_helper import load_pretrain
from importedsiammask.experiments.siammask_sharp.resnet import resnet50
import time
...
class Refine(nn.Module):
    def __init__(self):
        ...
        self.deconv = nn.ConvTranspose2d(256, 32, 36, 36)
        self.iconv1=nn.Sequential(nn.Conv2d(256,128,kernel_size=2),nn.ReLU())
        self.iconv2=nn.Sequential(nn.ConvTranspose2d(128,64,kernel_size=2,stride=2),nn.ReLU())
        self.iconv3=nn.Sequential(nn.Conv2d(64,32,kernel_size=3,padding=1),nn.ReLU())
    ...
    def forward(self, f, corr_feature, pos=None, test=False):
        ...
        if test:
            ...
            p0 = F.unfold(f[0], (144, 144), padding=0, stride=4).permute(0, 2, 1).contiguous().view(-1, 64, 144, 144)
            if not (pos is None): p0 = torch.index_select(p0, 0, pos)
            p1 = F.unfold(f[1], (72, 72), padding=0, stride=2).permute(0, 2, 1).contiguous().view(-1, 256, 72, 72)
            if not (pos is None): p1 = torch.index_select(p1, 0, pos)
            p2 = F.unfold(f[2], (36, 36), padding=0, stride=1).permute(0, 2, 1).contiguous().view(-1, 512, 36, 36)
            if not (pos is None): p2 = torch.index_select(p2, 0, pos)
        ...
        component1=self.h2(p3)
        component2=self.v2(p2)
        
        
        
        #TODOD eddit back
        out = self.post0(F.upsample(component1 + component2, size=(72, 72)))
        
        out = self.post1(F.upsample(self.h1(out) + self.v1(p1), size=(144, 144)))
        
        out = self.post2(F.upsample(self.h0(out) + self.v0(p0), size=(288, 288)))
        
        out=self.sig(out)
        
        out = out.view(-1, 288*288)
        '''out = self.post0(F.upsample( component2, size=(72, 72)))
        print(out)
        out = self.post1(F.upsample( self.v1(p1), size=(144, 144)))
        print(out)
        out = self.post2(F.upsample( self.v0(p0), size=(288, 288)))
        print(out)
        out = out.view(-1, 288*288)
        '''
        return out
